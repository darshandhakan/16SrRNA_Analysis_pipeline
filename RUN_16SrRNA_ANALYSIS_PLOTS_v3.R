  ...
  for (i in seq_along(GP[-length(GP)]))
  {
    ...
    for (j in seq_along(GP2))
    {
      if (GP[i] == GP2[j])
      {
        ...
        colnames(ALPHA_STATS_TABLE) <- c("Alpha Metric", paste0(GP[i], " [Mean]"), paste0(GP2[j], " [Mean]"), paste0(GP[i], " [Standard Deviation]"), paste0(GP[j], " [Standard Deviation]"), paste0(GP[i], " [SEM]"), paste0(GP[j], " [SEM]"), "FDR corrected p-value [Student's t-test]" ,"FDR corrected p-value [Wilcoxon's rank sum test]", "Paired t-test", "Paired Wilcoxon test")
        ...
        for (t in seq_along(TAXA))
        {
          ...
          else if (TAXA[t] == "Order")
          {
            ...
            p_abd <- plot_abundance(mm_aldex2, group = "Group") + scale_fill_manual(values = c("orangered", "dodgerblue")) + scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x))) + xlab(label = "Relative Normalized Abundance * 10^6 in Log10 scale") + theme(axis.text = element_text(size = 14, face = "bold"), legend.text = element_text(size = 12, face = "bold"), axis.title = element_text(size = 14, face = "bold"), legend.title = element_text(size = 14, face = "bold"))
